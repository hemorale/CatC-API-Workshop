...
def deploy(ip, tk, tmp_id, device):
    ...
    if os.path.exists(file_path):
        ...
        response = requests.post(url_template_apply, headers=headers, data=payload, verify=False)
        data = response.json()
...
with open('input.csv', newline='') as csvfile:
    ...
    template_name = input('\nEnter project name/template name from the above list : ')
    id_template, ver, template = get_template_id(dnac_ip,token,template_name)
    print(f'Template ID : {id_template}')
    if id_template == 0:
            raise ValueError('\nCannot find template:{0}'.format(template_name))
